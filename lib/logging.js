...
exports.log_act_err = function( instance, actinfo, actmeta, args, prior_ctxt, err ) {
  ...
  if (args && args.logErrors$ === true) {
    var errMsgWithDetails = {
      act: 'act',
      'actmeta.plugin_name': actmeta.plugin_name || '-',
      'actmeta.plugin_tag': actmeta.plugin_tag  || '-',
      OUT: 'OUT',
      'actinfo.actid': actinfo.actid,
      'actmeta.pattern': actmeta.pattern     || '-', 
      'actinfo.duration': actinfo.duration,
      'prior_ctxt.entry': prior_ctxt.entry ? 'ENTRY' : 
        'PRIOR;'+(prior_ctxt.chain.slice(0,prior_ctxt.depth)).join(','),
      'actmeta.id': 'A;'+actmeta.id, 
      'args.gate$': args.gate$ ? 'GATE' : '-',
    ...
    console.log(__filename, '::log_act_err::errMsgWithDetails');
    console.log('err', err);
    console.log(JSON.stringify(errMsgWithDetails, 0, 2));
  }
  instance.log.error(
    'act',
    actmeta.plugin_name || '-',
